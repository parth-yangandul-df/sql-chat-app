...
must_haves:
  truths:
    - "feature/langgraph-domain-tools branch exists and is the active branch"
...
<task type="auto">
  ...
  <action>
    1. Create and switch to branch `feature/langgraph-domain-tools` from current HEAD on `dev`:
       ```bash
       git checkout -b feature/langgraph-domain-tools
       ```
  ...
  <done>Branch feature/langgraph-domain-tools active. langgraph + langchain-core in pyproject.toml. GraphState importable with all keys. INTENT_CATALOG has exactly 24 entries (Resource×9, Client×5, Project×6, Timesheet×4).</done>
</task>
...
<verification>
- `git branch` shows `* feature/langgraph-domain-tools`
