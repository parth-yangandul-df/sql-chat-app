...
must_haves:
  truths:
    ...
    - "Conditional edge routes to run_domain_tool when domain is set, llm_fallback otherwise"
  artifacts:
...
graph.add_conditional_edges(
    ...
    {"run_domain_tool": "run_domain_tool", "llm_fallback": "llm_fallback"},
)
graph.add_edge("run_domain_tool", "interpret_result")
...
<task type="auto" tdd="true">
  <name>Task 2: Assemble the complete LangGraph StateGraph</name>
  <files>
    backend/app/llm/graph/graph.py
    backend/tests/test_graph_pipeline.py
  </files>
  <behavior>
    ...
    - The graph: classify_intent → conditional(run_domain_tool | llm_fallback) → interpret_result → write_history → END
  ...
  <action>
    1. Create `backend/app/llm/graph/graph.py`:
       ```python
       ...
       def _build_graph():
           ...
           graph.add_conditional_edges(
               ...
               route_after_classify,
               {
                   "run_domain_tool": "run_domain_tool",
    ...
    2. Fill `backend/tests/test_graph_pipeline.py`:
       ```python
       import pytest
       from unittest.mock import AsyncMock, MagicMock, patch
  ...
  <done>All test_graph_pipeline.py tests pass. get_compiled_graph() returns a usable compiled LangGraph. The domain tool path test passes with mocked nodes. Graph imports without side effects.</done>
</task>
...
<success_criteria>
All nodes implemented and tested. Compiled graph is invocable. Plan 05 can wire this into query_service.py.
</success_criteria>
