...
must_haves:
  truths:
    - "SQLServerConnector._run_query() accepts and passes params tuple to cursor.execute() — params are no longer silently ignored"
    - "BaseConnector.execute_query() abstract signature updated to params: tuple[Any, ...] | None = None"
  ...
  artifacts:
    - path: "backend/app/connectors/sqlserver/connector.py"
      provides: "Fixed _run_query() that passes params tuple to cursor.execute()"
      exports: ["SQLServerConnector"]
    - path: "backend/app/connectors/base_connector.py"
      provides: "Updated execute_query() abstract signature with params: tuple | None"
...
<task type="auto">
  <name>Task 0: Fix SQLServer connector params bug (critical — domain agents cannot work without this)</name>
  <files>
    backend/app/connectors/base_connector.py
    backend/app/connectors/sqlserver/connector.py
  </files>
  <action>
    **Bug:** `SQLServerConnector._run_query(self, sql: str)` calls `cursor.execute(sql)` with no params.
    The `params` argument accepted by `execute_query()` is completely ignored, making all parameterized queries silently wrong.

    1. In `backend/app/connectors/base_connector.py`, update the abstract `execute_query()` signature:
       ```python
       @abstractmethod
       async def execute_query(
           self,
    ...
    2. In `backend/app/connectors/sqlserver/connector.py`:
       - Update `_run_query()` signature to accept params:
         ```python
         def _run_query(self, cursor, sql: str, params: tuple | None = None) -> list:
         ```
       - Change the `cursor.execute(sql)` call to pass params when present:
         ```python
         if params:
             ...
             cursor.execute(sql)
         ```
       - Update `execute_query()` to pass params through to `_run_query()`:
         ```python
         async def execute_query(
             self,
             sql: str,
         ...
         ```
         And wherever `_run_query` is called internally, pass `params` through.
  </action>
  <verify>
    <automated>cd backend && python -c "from app.connectors.sqlserver.connector import SQLServerConnector; import inspect; sig = inspect.signature(SQLServerConnector.execute_query); print('params' in sig.parameters and 'OK' or 'FAIL')"</automated>
  </verify>
  <done>SQLServerConnector.execute_query() and _run_query() accept params tuple. BaseConnector abstract signature updated. Parameterized queries will no longer silently ignore parameters.</done>
</task>
