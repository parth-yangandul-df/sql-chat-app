...
from app.llm.graph.intent_catalog import (
    INTENT_CATALOG,
    ensure_catalog_embedded,
    get_catalog_embeddings,
)
from app.llm.graph.nodes.classifier_keywords import (
    _STOP_WORDS,
    _has_first_person,
    _keyword_route,
)
...
async def _semantic_followup_check(question: str, last_turn_context: dict | None) -> bool:
    ...
    try:
        ...
        logger.warning(
            "semantic_followup: embedding failed (%s) — falling back to word heuristics", e
        )
...
async def classify_intent(state: GraphState) -> dict[str, Any]:
    ...
    if best_entry.domain == "user_self" and not _has_first_person(question):
        ...
        user_self_indices = [i for i, e in enumerate(INTENT_CATALOG) if e.domain == "user_self"]
        masked_scores = [s if i not in user_self_indices else -1.0 for i, s in enumerate(scores)]
