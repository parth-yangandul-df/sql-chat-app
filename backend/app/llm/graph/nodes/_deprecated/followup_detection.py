...
async def followup_detection_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    last_intent = state.get("last_turn_context", {}).get("intent") if state.get("last_turn_context") else None
    current_filters = state.get("filters", [])
    last_filters = state.get("last_turn_context", {}).get("filters") if state.get("last_turn_context") else None
